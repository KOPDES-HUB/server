...
export const getById = async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.kopdes_hub_sch_profil_koperasi.findUnique({
      where: {
        koperasi_ref,
      },
    });
...
export const create = async (req: Request, res: Response) => {
  try {
    ...
    const exist = await prisma.kopdes_hub_sch_profil_koperasi.findUnique({
      where: {
    ...
    if (exist) {
      return errorResponse(res, "Profil koperasi sudah ada", 409);
    }

    const data = await prisma.kopdes_hub_sch_profil_koperasi.create({
      data: body,
    });

    return successResponse(res, "Berhasil membuat data", data, 201);
  } catch (error) {
    return errorResponse(res, "Terjadi kesalahan", 500, error);
  }
};
...
export const edit = async (req: Request, res: Response) => {
  try {
    ...
    const data = await prisma.kopdes_hub_sch_profil_koperasi.update({
      where: {
        koperasi_ref,
      },
      data: {
    ...
    return successResponse(res, "Berhasil memperbarui data", data);
  } catch (error) {
    return errorResponse(res, "Terjadi kesalahan", 500, error);
  }
};
...
export const checkLocation = async (req: Request, res: Response) => {
  try {
    ...
    const koperasi = await prisma.kopdes_hub_sch_profil_koperasi.findMany({
      where: {
    ...
    return successResponse(res, "Lokasi koperasi terdekat", result);
